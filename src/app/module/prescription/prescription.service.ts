...
const givePrescription = async (user: IRequestUser, payload: ICreatePrescriptionPayload) => {
  ...
  if (appointmentData.doctorId !== doctorData.id) {
    throw new AppError(status.BAD_REQUEST, "You can obly give prescription for your own appointments");
  }
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const pdfBuffer = await generatePrescriptionPDF({
      ...
      appointmentDate: appointmentData.appointmentStart || appointmentData.schedule?.startDateTime || new Date(),
    ...
    try {
      ...
      await sendEmail({
        ...
        templateData: {
          ...
          appointmentDate: new Date(appointmentData.appointmentStart || appointmentData.schedule?.startDateTime || new Date()).toLocaleString(),
...
const updatePrescription = async (user: IRequestUser, prescriptionId: string, payload: any) => {
  ...
  const pdfBuffer = await generatePrescriptionPDF({
    ...
    appointmentDate: prescriptionData.appointment.appointmentStart || prescriptionData.appointment.schedule?.startDateTime || new Date(),
  ...
  try {
    await sendEmail({
      ...
      templateData: {
        ...
        prescriptionId: result.id,
        appointmentDate: new Date(result.appointment.appointmentStart || result.appointment.schedule?.startDateTime || new Date()).toLocaleString(),
