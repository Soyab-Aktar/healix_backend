...
const createMyDoctorSchedule = async (user: IRequestUser, paylaod: ICreateDoctorSchedulePayload) => {
...
const updateMyDoctorSchedule = async (user: IRequestUser, paylaod: IUpdateDoctorSchedulePayload) => {
  const doctorData = await prisma.doctor.findUniqueOrThrow({
    where: {
      email: user.email
    }
